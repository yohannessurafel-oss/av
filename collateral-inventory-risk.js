/* ═══════════════════════════════════════════════════════════
   Africa Village Microfinance — Collateral Inventory Risk
   06-collateral-inventory-risk.js  v2.0
═══════════════════════════════════════════════════════════ */
...
async function loadBranches() {
  ...
  try {
    ...
    const sel2 = document.getElementById('collateralBranchId');
    if (!sel2) return;
...
document.getElementById('collateralBranchId')?.addEventListener('change', function () {
  const nameEl = document.getElementById('collateralBranchName');
...
function setMode(mode) {
  ...
  if (view) {
    view.querySelectorAll('input:not([readonly]), select, textarea').forEach(el => {
      if (el.dataset.alwaysEnabled !== undefined || el.id === 'collateralBranchId') { el.disabled = false; return; }
      el.disabled = !isEdit;
    });
  }
  document.querySelectorAll('input[readonly]').forEach(el => el.disabled = false);
  document.getElementById('collateralBranchId').disabled = false;
