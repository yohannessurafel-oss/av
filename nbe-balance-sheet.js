...
async function runReport() {
  ...
  try {
    ...
    // Grand total: Liability + Capital ending balances, PLUS Net Income.
    // Net Income (Income roots' ending minus Expense roots' ending) is
    // not folded into any Capital/Equity account balance, because no
    // formal period-close entry has ever been posted in this system —
    // every Equity account confirmed at 0.00 earlier in this audit.
    // Comparing Assets against Liability+Capital alone would therefore
    // report "out of balance" by exactly the net income amount on any
    // period with real income/expense activity — not a real accounting
    // error, just an unclosed period. Surfacing it as an explicit line
    // (same principle as the overpayment-suspense fix in
    // post_loan_repayment_v4.sql) instead of hiding the mismatch.
    ...
    const incomeRoots = roots.filter(r => r.account_type === 'INCOME');
    const expenseRoots = roots.filter(r => r.account_type === 'EXPENSE');
    const netIncome = incomeRoots.reduce((s, r) => s + r._totals.end, 0)
                     - expenseRoots.reduce((s, r) => s + r._totals.end, 0);
    const liabCapTotal = (liabRoot ? liabRoot._totals.end : 0) + (capRoot ? capRoot._totals.end : 0) + netIncome;

    rowsHtml.push(`
      <tr class="bs-row-header">
        <td style="padding-left:8px;">Net Income (Current Period — not yet closed to Capital)</td>
        <td></td><td></td><td class="num"></td><td class="num"></td><td class="num"></td>
        <td class="num">${fmt(netIncome)}</td>
      </tr>
    `);
    rowsHtml.push(`
      <tr class="bs-final-total">
        <td colspan="6">LIABILITY &amp; CAPITAL (incl. unclosed Net Income)</td>
