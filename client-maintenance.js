/* ═══════════════════════════════════════════════════════════
   Africa Village Microfinance — 13 Client Financial Account Maintenance
   account-maintenance.js  v1.2
═══════════════════════════════════════════════════════════ */
...
let _currentAccountNumber = null; 
...
async function viewRecord() {
  ...
  if (!acctNo && !clientId) { toast('Enter an Account Number or Client ID to search.', 'warning'); return; }
  try {
    if (acctNo) { await viewAccountByNumber(acctNo); } 
    else { await verifyClient(clientId); }
...
async function deleteRecord() {
  ...
  try {
    await sbFetch(`${TABLE}?account_number=eq.${encodeURIComponent(_currentAccountNumber)}`, {
      method: 'PATCH', prefer: 'return=minimal', body: JSON.stringify({ account_status: 'Closed' })
    });
...
document.getElementById('accountNumber')?.addEventListener('keydown', e => { if (e.key === 'Enter') viewRecord(); });
...
document.getElementById('btnGlobalAdd')?.addEventListener('click', () => {
  clearForm(); setMode('add'); document.getElementById('accClientId')?.focus();
