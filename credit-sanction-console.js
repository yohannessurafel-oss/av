/* ═══════════════════════════════════════════════════════════
   Africa Village Microfinance — Credit Sanction Console
   04-credit-sanction-console.js  v2.0
═══════════════════════════════════════════════════════════ */
...
async function loadBranches() {
  ...
  try {
    ...
    const sel2 = document.getElementById('sanctionBranchId');
    if (!sel2) return;
...
function setMode(mode) {
  ...
  if (view) {
    view.querySelectorAll('input:not([readonly]), select, textarea').forEach(el => {
      if (el.dataset.alwaysEnabled !== undefined || el.id === 'sanctionBranchId') { el.disabled = false; return; }
      el.disabled = !isEdit;
    });
  }
  document.querySelectorAll('input[readonly]').forEach(el => el.disabled = false);
  document.getElementById('sanctionBranchId').disabled = false;
